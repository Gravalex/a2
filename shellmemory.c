...
#define VARIABLE_STORE_SIZE 100
#define FRAME_SIZE 3
...
//Where the FRAM STORE and the VARIABLE store are;
...
void mem_init(){
	...
	for (i=0; i<FRAME_STORE_SIZE; i++){
		shellmemory[i].var = "none";
		shellmemory[i].value = "none";
		shellmemory[i].isFrame = true;
	}
	for (i=FRAME_STORE_SIZE; i<SHELL_MEM_LENGTH; i++){
		shellmemory[i].var = "none";
		shellmemory[i].value = "none";
		shellmemory[i].isFrame = false;
	}
